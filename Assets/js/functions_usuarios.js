...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(document.querySelector("#formUsuario")){
        ...
        formUsuario.onsubmit = function(e) {
            ...
            let intCel = document.querySelector('#txtCel').value;
            let intTipousuario = document.querySelector('#listRolid').value;
            let strPassword = document.querySelector('#txtPassword').value;
            let intStatus = document.querySelector('#listStatus').value;

            let strSexo = document.querySelector('#txtSexo').value;
            let strDireccion = document.querySelector('#txtDireccion').value;
            let strUserAcces = document.querySelector('#txtUserAcces').value;
          

            if(strCi == '' || strApellido == '' || strNombre == '' || strEmail == '' || intCel == '' || intTipousuario == ''|| strSexo == ''|| strDireccion == '')
            {
                ...
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status)
                    {
                        if(rowTable == ""){
                            ...
                            rowTable.cells[4].textContent = intCel;
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
               let estadoUsuario = objData.data.status == 1 ? 
                ...
                document.querySelector("#celCel").innerHTML = objData.data.cel;
                document.querySelector("#celEmail").innerHTML = objData.data.email;
                document.querySelector("#celTipoUsuario").innerHTML = objData.data.nombrerol;
                document.querySelector("#celEstado").innerHTML = estadoUsuario;
                document.querySelector("#celFechaRegistro").innerHTML = objData.data.fecha_reg; 

                document.querySelector("#celDireccion").innerHTML = objData.data.direccion_dom; 
                document.querySelector("#celSexo").innerHTML = objData.data.sexo; 
                document.querySelector("#celUserAcces").innerHTML = objData.data.usuario; 
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector("#txtCel").value = objData.data.cel;
                ...
                document.querySelector("#txtUserAcces").value =objData.data.usuario;

                $('#listRolid').selectpicker('render'); //Prepara la lista del tipo de usuario a registrar 
...
// Consulta las pensiones del estudiante
function buscarPensionesCi(ci)
{    // Consultar Pensiones de estudiante
    if(document.querySelector("#PensionesEstudianteCi")){
        let Ci =ci;
        if(Ci==0){
            swal("Atención", "Ci ERROR." , "error");
            return false;
        }
        divLoading.style.display = "flex";
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        let ajaxUrl = base_url+'/Usuarios/getPensiones/'; 
